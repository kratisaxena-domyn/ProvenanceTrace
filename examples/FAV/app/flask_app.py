...
AGENT_LOG_DIR = os.path.join(os.path.dirname(__file__), '../agent_logs')
AGENT_LOG_FILES = [
...
Normalize_Index_Retrieve_obj = Normalize_Index_Retrieve()
normalize_end = time.time()

halluc_start = time.time()
HallucinationClaimSupport_obj = HallucinationClaimSupport_advanced_withMapping(threshold=0.5)
halluc_end = time.time()

init_end = time.time()

ic(init_end - init_start)
# Load spaCy model for sentence segmentation (same as used in extract_atomic_facts_with_mappings)
spacy_load_start = time.time()
try:
    nlp = spacy.load("en_core_web_sm")
except OSError:
    # If model not found, try to use basic English model
    nlp = spacy.load("en_core_web_sm")
# In-memory chat history (for demo; use session or DB for prod)
chat_history = []

# Create a single persistent orchestrator instance
orch_start = time.time()
orchestrator = ReActOrchestrator()
orch_end = time.time()

ic(orch_end - orch_start)
# --- HTML Template ---
TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
...
def chat():
    ...
    # Use the persistent orchestrator instance
    global orchestrator
    ...
    # Trace pipeline - use the current turn number from orchestrator
    turn_number = orchestrator.turn_number - 1  # orchestrator increments after run, so subtract 1

    trace_start = time.time()

    result = extract_turn_trace(turn_number, AGENT_LOG_DIR, AGENT_LOG_FILES)
    trace_end = time.time()
    
    answer_from_trace = result.get("answer", "")
    
    # Fallback to orchestrator answer if trace answer is empty or None
    if not answer_from_trace:
        answer_from_trace = answer

    trail = result.get("steps", [])
    ...
    matched_docs = Normalize_Index_Retrieve_obj.return_matching_docs(entities, trail, answer_from_trace, sbert_model) if entities else []
    docs_end = time.time()
    
    n = len(matched_docs)
    top_half = matched_docs[:max(1, n//2)] if n > 0 else []
    
    # Only proceed with fact extraction if we have a valid answer
    if answer_from_trace and answer_from_trace.strip():
        try:
            # Fact extraction and hallucination check
            ...
            # Only proceed with hallucination check if we have facts and matched docs
            if fact_mappings and matched_docs:
                halluc_start = time.time()
                hallucination_results = HallucinationClaimSupport_obj.check_claims(fact_mappings, matched_docs, sbert_model)
                halluc_end = time.time()
            else:
                hallucination_results = []
            
            # Map facts to sentences - Initialize with proper sentence count
            mapping_start = time.time()
            doc = nlp(answer_from_trace)
            text_sents = [sent.text.strip() for sent in doc.sents if len(sent.text.split()) > 4]
        ...
        # No valid answer, use empty results
        hallucination_results = []
        facts_by_sentence = []
        fact_mappings = []
        answer_from_trace = "Sorry, I couldn't generate a proper response."
    
    # Highlight answer
    highlight_start = time.time()
    answer_html = highlight_answer(answer_from_trace, hallucination_results, facts_by_sentence, len(chat_history))
